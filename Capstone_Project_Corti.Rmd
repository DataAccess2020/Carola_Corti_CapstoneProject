...
-   How did the Covid-19 spread appear within this discourse?
...
```{r}
knitr::kable(head(dfm_lookup(myDfm, dict)[,1]))
...
```{r}
knitr::kable(head(dfm_lookup(myDfm, dict)[,2]))
...
```{r}
knitr::kable(head(dfm_lookup(myDfm, dict)[,3]))
...
```{r}
knitr::kable(head(dfm_lookup(myDfm, dict)[,4]))
```
...
In spite of the analyses I run, no difference in lexical and sentiment terms, appear to occur between the corpora I've analyzed.
